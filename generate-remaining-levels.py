"""
Generate content for remaining DSA levels with rate limit handling
Targets levels: 6, 8, 9, 11, 12, 13, 14, 16, 17, 18, 19
"""
...
class DSALevelGenerator:
    ...
    def call_gemini_api(self, prompt: str, retries: int = 3) -> str:
        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code == 429:
                    ...
                    print(f"⚠️  Rate limit hit. Waiting {wait_time} seconds before retry {attempt + 1}/{retries}...")
...
def main():
    """Main function to update remaining DSA levels"""
    generator = DSALevelGenerator()
    
    print("\n" + "="*70)
    print("🎓 DSA COURSE CONTENT GENERATOR - REMAINING LEVELS")
    print("="*70)
    ...
    # Remaining levels to update
    levels_to_update = [6, 8, 9, 11, 12, 13, 14, 16, 17, 18, 19]
    
    print(f"\n📚 Updating {len(levels_to_update)} remaining levels...")
    ...
    for i, level_id in enumerate(levels_to_update):
        ...
        if new_content:
            ...
            # Update description from first concept
            if new_content.get("concepts") and len(new_content["concepts"]) > 0:
                first_concept_content = new_content["concepts"][0].get("content", "")
                if first_concept_content:
                    current_level["description"] = first_concept_content[:150] + "..." if len(first_concept_content) > 150 else first_concept_content
            
            successful_updates += 1
        else:
            failed_updates.append(level_id)
        
        # Add longer delay between requests to avoid rate limiting
        if i < len(levels_to_update) - 1:
            wait_time = 8
    ...
    try:
        ...
        print("="*70)
        print(f"\n📊 Summary:")
        print(f"   - Successful Updates: {successful_updates}/{len(levels_to_update)}")
        if failed_updates:
            print(f"   - Failed Levels: {failed_updates}")
        print(f"   - Total Levels in Config: {len(levels)}")
